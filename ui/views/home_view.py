...
class HomeView:
    ...
    def _recent_projects(self):
        """Build the compact Home preview for recent work.

        The Home dashboard intentionally shows only the newest three real
        session items.  The full session history is kept in
        ``self._recent_results`` and is available through View all.
        """
        ...
        self.projects_grid = ft.ResponsiveRow(
            spacing=14,
            run_spacing=14,
        )
        self._rebuild_project_cards()

        view_all_button = ft.TextButton(
            "View all",
            icon=ft.Icons.GRID_VIEW_ROUNDED,
            on_click=self._show_all_recent_projects,
            style=ft.ButtonStyle(color="#7292FF"),
        )

        return ft.Container(
            padding=14,
            bgcolor="#D907101D",
            border=ft.Border.all(1, "#263650"),
            border_radius=16,
            content=ft.Column(
                spacing=10,
                controls=[
    ...
    def _rebuild_project_cards(self):
        """Refresh the Home preview without throwing away history."""
        # Permanent demo cards always stay first on Home.
        demo_cards = [
            ft.Container(
                col={"xs": 12, "md": 4},
        ...
        # Home is only a preview.  Keep all session items in memory, but render
        # just the newest three here so the dashboard does not grow forever.
        visible_recent = self._recent_results[:3]
        recent_cards = [
            ft.Container(
                col={"xs": 12, "md": 4},
                content=self._project(**project),
            )
            for project in visible_recent
        ]
    ...
    def add_recent_project(
        ...
        """Add an opened/processed image to the complete session history."""
        ...
        self._recent_results.insert(
            0,
            {
                ...
                "edited": (
                    f"{timestamp}  •  "
                    f"{source_name or 'Untitled image'}"
                ),
        ...
        # IMPORTANT: do not truncate self._recent_results here.
        # The Home preview shows [:3], while View all displays the whole list.
        self._rebuild_project_cards()
    ...
    def _show_all_recent_projects(self, e=None):
        """Open a scrollable gallery containing all real session work."""
            ...
            body = ft.Container(
                ...
                content=ft.Column(
                    ...
                    controls=[
                        ...
                        ft.Text(
                            "Open or process an image and it will appear here.",
                            size=10,
                            color=AppColors.MUTED,
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Container(
                ...
                content=ft.Column(
                    ...
                    controls=[
                                    ...
                                    controls=[
                                        ...
                                        ft.Column(
                                            ...
                                            controls=[
                                                ...
                                                ft.Text(
                                                    f"{count} {item_word} from this session",
                                                    size=9,
                                                    color=AppColors.MUTED,
                                                ),
                                            ],
                        ...
                        ft.Divider(height=1, color="#263650"),
            ...
            actions=[
                ft.TextButton(
                    ...
                    style=ft.ButtonStyle(color="#8EA6FF"),
                    on_click=lambda ev: self._close_recent_projects(dialog, ev),
                ),
            ],
            actions_alignment=ft.MainAxisAlignment.END,
        )

        self._recent_dialog = dialog

        try:
            page.open(dialog)
        except Exception:
            # Compatibility fallback for older Flet versions.
            try:
    ...
    def _close_recent_projects(self, dialog, e=None):
        """Close the View all dialog on both newer and older Flet versions."""
        page = getattr(e, "page", None) if e is not None else None
        if page is None:
            page = getattr(self.control, "page", None)
        if page is None:
